...
exports.addProfessorItem = async (req, res) => {
    ...
    try {
        ...
        if (isAfter(newStartTime, newEndTime) || isEqual(newEndTime, newStartTime)) {
            return sendError(res, ERRORS.ERROR_INVALID_START_TIME)
        }
...
exports.removeProfessorItem = async (req, res) => {
    ...
    try {
        ...
        const itemIdx = await ProfessorItem.findOne({ _id: itemId, professor_id: userId });
        if (itemIdx === -1) {
        ...
        await ProfessorItem.updateOne({ _id: itemId }, { active: ACTIVE.NOT_ACTIVE });
        ...
        // })
        await StudentItem.updateMany({ professor_item_id: itemIdx }, { active: ACTIVE.NOT_ACTIVE });
...
exports.removeProfessorRecurringItem = async (req, res) => {
    ...
    try {
        ...
        for (const item of items) {
            ...
            await StudentItem.updateMany({ professor_item_id: item._id }, { active: false });
        }
...
exports.getItems = async (req, res) => {
    ...
    try {
        ...
        const profItems = await ProfessorItem.find({ professor_id: professorId, active: ACTIVE.ACTIVE });

        // const studItems = studentItems.filter(item => item.professor_id === professorId && item.active === ACTIVE.ACTIVE)
        const studItems = await StudentItem.find({ professor_id: professorId, active: ACTIVE.ACTIVE });
...
exports.addStudentItem = async (req, res) => {
    ...
    try {
        ...
        if (isAfter(newItemStartTime, newItemEndTime) || isEqual(newItemEndTime, newItemStartTime)) {
            return sendError(res, ERRORS.ERROR_INVALID_START_TIME)
        }
        ...
        if ((isAfter(newItemStartTime, profDateStart) || isEqual(newItemStartTime, profDateStart)) &&
            (isBefore(newItemEndTime, profDateEnd) || isEqual(newItemEndTime, profDateEnd))) {
            // NOW CHECK IF THE NEW TIME FRAME IS NOT OVERLAPPING WITH THE OTHER STUDENTS TIME FRAMES
            if (studItems.every(studItem => {
                studItemStartTime = new Date(studItem.startTime)
                studItemEndTime = new Date(studItem.endTime)
                return (
                    (isAfter(newItemStartTime, studItemEndTime) || isEqual(newItemStartTime, studItemEndTime)) || (
                        (isBefore(newItemEndTime, studItemStartTime) || isEqual(newItemEndTime, studItemStartTime))))
            })) {
...
exports.removeStudentItem = async (req, res) => {
    ...
    try {
        ...
        if (verifiedToken.user.occupation === OCCUPATION_STATUS.STUDENT && itemIdx.student_id !== userId) {
            return sendError(res, ERRORS.ERROR_ITEM_CANNOT_BE_DELETED)
        }
